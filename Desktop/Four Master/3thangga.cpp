...
int main()
{
    ...
    tree<pair<int,int>, null_type, less<pair<int,int>>, rb_tree_tag, tree_order_statistics_node_update> ts;
    int res = 0;
    for (int x = k; x <= min(n,k+5); x++){
        for (int i = 1; i <= n; i++){
            ts.insert({a[i],i});
            if (i >= x){
                for (auto x : ts) cout << x.first << " "; cout << "\n";
                auto g = ts.find_by_order((x-1)/2);
                res = max(res, (*g).first);
                ts.erase({a[i - x + 1], i - x + 1});
            }
        }
        ts.clear();
    }
